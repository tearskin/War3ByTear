...
//自定义复选框等等判断布尔
bool infineall = false;
void DrawMainWin()
{
   // DrawBox(200, 200, 150, 150, color_red);
    //初始化内置绘图
    const auto draw = ImGui::GetBackgroundDrawList();
    //draw->AddText(ImVec2(200,200), 3.0f, color_red, 0, "tear hack for warcraftIII");
    draw->AddText(ImVec2(100, 0),color_red, "you can  join qqGroup:435539500 or Telegram:t.me/tearWorld");
   // draw->AddLine(ImVec2(50, 50), ImVec2(200, 200), color_blue, 2.0f);
    ImGuiIO& io = ImGui::GetIO(); (void)io;
    //加载中文字体
   // io.Fonts->AddFontFromFileTTF("D:\\msyh.ttc", 13.0f, NULL, io.Fonts->GetGlyphRangesChineseSimplifiedCommon());
    //io.Fonts->AddFontFromFileTTF("D:\\simhei.ttf", 18, nullptr, io.Fonts->GetGlyphRangesChineseFull());
    // 加载字体中的所有汉字  ？？ 汉字显示不全
   ...
   /* io.WantCaptureMouse = p_open;
    ...
    if (p_open)
    {
        ...
        ImGui::Begin(u8"tear War3 Game");
        if (ImGui::Button("_asm CallTest"))
        {
            char* title = "cannot use Chinese ";
            char* val = "used from github source code";
        ...
        //zhege 这里其实就可以开始添加自己的内容了
        ImGui::SameLine();
        ImGui::Spacing();
        if (ImGui::Button("Unlimited article(goods)")) {
            UnlimitedThings();
     }
        if (ImGui::Button("thousand Level!")) {
            immediateExp();
        }
        if (ImGui::Button("10w coin!")) {
           coin();
        }
        if (ImGui::Button("NoCoolDown")) {
            CD();
        }
        ImGui::Checkbox(u8"openTestDraw", &draw_demo);
    ...
    if (draw_demo)
    {
        ...
        draw->AddText(ImVec2(100, 100), current_color, "hello I am  tear BiLiBiLi:deepDream...");
  ...
  /* ImFontConfig font_cfg;
    ...
    */
    ...
    if (greetings)
    {
        ...
        ImGui::Begin("tear_War3", &greetings, ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoInputs);
        ImGui::Text("plugn load success,press [insert] open menu");
        ImGui::End();
